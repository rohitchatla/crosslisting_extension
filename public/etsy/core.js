  ...
  setTimeout(() => {
    chrome.storage.sync.get("data", async (value) => {
      ...
      setTimeout(async () => {
        ...
        $("#price_retail-input")
          .val(value.data.price.replace(/[^\d.]/, ""))
          .dispatch("input");
        console.log(value.data.price.replace(/[^\d.]/, ""));
        document.getElementById("quantity_retail-input").value =
          value.data.quantity;
        ...
        if (value.data.material) {
          (async function () {
            ...
            let tags = value.data.material
              .split(/\s*,\s*/)
              .map((v) => v.replace(/[^\w\s\d]/g, ""));
            let deleteButtons = tagInput
              .parents(".col-sm-8")
              .find(".tag-close button");
            for (let i = 0; i < deleteButtons.length; i++)
              deleteButtons.eq(i).dispatch("click");
            for (let tag of tags) {
        ...
        if (value.data.colorShade) {
          (async function () {
            let colorSelect;
            while (!colorSelect)
              colorSelect = await waitForNode("#attribute-2");
        ...
        if (value.data.size) {
          (async function () {
            let sizeNode;
            while (!sizeNode)
              sizeNode = await waitForNode('legend.strong:contains("Size")');
            sizeNode = sizeNode.parents(".col-flush");
            let sizeType = sizeNode.find('[name="Scales"]');
            if (sizeType.length) {
              sizeType
                .val(sizeType.find('option:not([value=""])').eq(0).val())
                .dispatch("change");
              await wait(1);
            }
        ...
        if (value.data.style) {
          (async function () {
            ...
            for (let opt of opts) {
              if (
                $(opt).text().toLowerCase() === value.data.style.toLowerCase()
              ) {
        ...
        if (value.data.weightLB || value.data.weightOZ) {
          (async function () {
            ...
            $("#weight_primary")
              .val(value.data.weightLB.replace(/[^\d.]/g, ""))
              .dispatch("blur");
            $("#weight_secondary")
              .val(value.data.weightOZ.replace(/[^\d.]/g, ""))
              .dispatch("blur");
          })().then();
        }
        if (
        ...
        if (dt.zip) {
          let setZipCode = async function () {
            (await waitForNode('[name="origin_postal_code"]')).val(
              value.data.zipCode
            );
            setZipCode().then();
          };
