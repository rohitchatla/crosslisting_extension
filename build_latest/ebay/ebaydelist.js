...
function ebaydelist(url) {
  setTimeout(() => {
    setTimeout(() => {
      ...
      console.log(len);
      for (let i = 0; i < len; i++) {
        //m0-0-4-16-45-gridData-gridContent-grid-grid-row-203043540349-16[2]-1[0]
        console.log(i);
        if (document.getElementsByTagName("a")[i].href[7] == url[8]) {
          console.log(document.getElementsByTagName("a")[i].href);
          var f =
            "s0-0-4-16-45-gridData-gridContent-grid-grid-row-" +
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (document.getElementsByTagName("a")[i].href[7] == url[8]) {
          ...
          var f =
            ...
            url.substring(url.lastIndexOf("/") + 1) +
            "-grid-menu";
          document.getElementById(f).children[0].click(); //button
        }
      }
    }, 2000);
  }, 2000);
